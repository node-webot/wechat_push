...
Pusher.prototype.login = function (verify, callback) {  // verify = {code: code, cookie: cookie}
  ...
  function fetchWorker() {
    ...
    var fetchUrl = 'http://admin.wechat.com/cgi-bin/getmessage?t=wxm-message&token=' + that.token + '&lang=en_US&count=50';
    ...
    that.fetchWorkerId = setInterval(function () {
      urllib.request(fetchUrl, {headers: headers, timeout: 10000, type: 'GET'}, function (err, data, res) {
        ...
        if (err) {
          ...
          var jsonStr = cheerio.load(data.toString())('#json-msgList').html();
          var fetched = jsonlib.parse(jsonStr);
          if (that.newestMessageID) {
            message = fetched.filter(function(msg){
              return (parseInt(msg.id, 10) > that.newestMessageID);
            });
          } else {
            message = fetched;
          }
          that.newestMessageID = parseInt(fetched[0].id, 10);
          if (message.length > 0) {
